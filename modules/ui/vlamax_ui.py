# ## Śledzenie Zmian VLamax w Czasie (TAC w Praktyce)
#
# Jedną z najpotężniejszych zastosowań koncepcji TAC jest śledzenie wartości VLamax w czasie.
# Regularne pomiary pozwalają zobaczyć, jak trening wpływa na twoją zdolność glikolityczną:
#
# - Dla sportowców wytrzymałościowych: systematyczne spadki VLamax wskazują na skuteczny trening aerobowy
# - Dla sprinterów: kontrolowane wzrosty VLamax pokazują rozwój mocy anaerobowej
# - Dla zawodników wszechstronnych: utrzymanie stabilnego VLamax przy jednoczesnym wzroście VO2max wskazuje na optymalny balans metaboliczny
#
# Sugerowana częstotliwość pomiaru: co 4-6 tygodni podczas okresów podstawowych i przygotowawczych,
# oraz co 2-3 tygodnie podczas okresów specjalistycznych i wyścigowych.
...
def _render_current_profile(profile: VLamaxProfile) -> None:
    ...
    metric("PDGE", f"{profile.vlamax:.3f}", column=col1, suffix=" mmol/l/s")
...
def _render_rider_classification(profile: VLamaxProfile) -> None:
    ...
    st.markdown(
        ...
        f"<b>Typ kolarza:</b> {profile.rider_type} (PDGE = {profile.vlamax:.3f} mmol/l/s)"
        f"</div>",
...
def _render_theory() -> None:
    with st.expander("📖 Teoria — VLamax i Profil Metaboliczny"):
        ...
        **VLamax** (maksymalna szybkość produkcji mleczanu) określa profil metaboliczny kolarza:
        ...
        | Typ | PDGE (mmol/l/s) | Charakterystyka |
        |-----|-------------------|-----------------|
        ...
        **Climber** | < 0.45 | Wysoki VO2max, wytrzymałość na podjazdach |

        **Ograniczenia:** Estymacja z PDC ma niepewność ±15-20%.

        ## Kluczowe Koncepcje w Tri_Dashboard

        ### PDGE (Power-Derived Glycolytic Estimate)
        Wartość VLamax w Tri_Dashboard jest szacowana na podstawie krzywej mocy-czas (W'/CP ratio) z korekcją PCR, a nie pomiaru bezpośredniego mleczanu. Oznacza to, że mierzymy pochodną zdolności glikolitycznej, a nie samą glikolizę.

        ### GTE (Glycolytic Trade-off Effect)
        Wysoka VLamax korzystnie wpływa na wysiłki krótkoterminowe (sprinty), ale może ograniczać wydolność wytrzymałościową poprzez zwiększoną produkcję mleczanu i zmniejszoną efektywność tlenową.

        ### MSV (Metabolic Stimulus Variation)
        Dwóch sportowców o identycznym VO2max ale różnej VLamax będzie odbierało ten sam trening interwałowy zupełnie inaczej - wpływ na wykorzystanie dostępnego tlenu podczas wysiłku progowego.

        ### TAC (Trainable Anaerobic Capacity)
        Wbrew przekonaniu o stałej cechach genetycznych, VLamax jest znacznie podatne na trening i może być celowo zwiększane (dla sprinterów) lub zmniejszane (dla wytrzymałościowców) poprzez odpowiednio skoncentrowany trening.
        """)
