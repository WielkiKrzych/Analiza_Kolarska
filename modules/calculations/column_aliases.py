...
def resolve_hrv_column(df: pd.DataFrame) -> str | None:
    """Rename first matching HRV/RMSSD alias to canonical 'hrv'. Returns 'hrv' or None."""
    if "hrv" in df.columns:
        return "hrv"
    for alias in HRV_ALIASES:
        if alias in df.columns:
            df.rename(columns={alias: "hrv"}, inplace=True)
            return "hrv"
    return None
